...
class TimeDeposit:
    ...
    def __str__(self):
        ...
        Сумма:              [1,000; 100,000)
        """
        newstr = 'Наименование:       ' + self.name
        newstr += '\nВалюта:             ' + self.currency
        newstr += '\nПроцентная ставка:  ' + str(self._interest_rate)
        newstr += '\nСрок (мес.):        [{}; {})'\
                  .format(self._period_limit[0], self._period_limit[1])
        newstr += '\nСумма:              [{:,}; {:,})'\
                  .format(self._sum_limit[0], self._sum_limit[1])
        return newstr
...
class BonusTimeDeposit(TimeDeposit):
    ...
    def __str__(self):
        ...
        newstr += '\nБонус (%):          ' + str(self._bonus['percent'])
        newstr += '\nБонус (мин. сумма): {:,}'.format(self._bonus['sum'])
        return newstr
    ...
    def _check_self(self):
        ...
        assert 0 < self._bonus['percent'] <= 100, \
            "Неверно указан бонусный процент по вкладу!"
        assert 0 < self._bonus['sum'], \
            "Неверно указана минимальная сумма бонуса!"
    ...
    def get_profit(self, initial_sum, period):
        ...
        начисляется бонус.
        """
        newprofit = super().get_profit(initial_sum, period)
        if initial_sum > self._bonus['sum']:
            newprofit *= (1 + self._bonus['percent'] / 100)
        return newprofit
...
class CompoundTimeDeposit(TimeDeposit):
    ...
    def get_profit(self, initial_sum, period):
        ...
        """
        self._check_user_params(initial_sum, period)
        return initial_sum * (1 + self._interest_rate
                              / 100 / 12) ** period - initial_sum
...
deposits = (
    ...
    BonusTimeDeposit("Супербонус", **deposits_data,
                     bonus=dict(percent=10, sum=5000)),
    TimeDeposit("Долгий", interest_rate=7,
                period_limit=(12, 24),
                sum_limit=(1000, 100000)),
)
