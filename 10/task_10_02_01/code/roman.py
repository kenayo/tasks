...
class Roman:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, int):
            ...
            raise TypeError('Неверный формат чисел')
    ...
    def __check_arabic(value):
        ...
        if value < Roman.ARABIC_MIN:
            raise ValueError('Введенное число меньше минимального',
                             Roman.ARABIC_MIN)
        if value > Roman.ARABIC_MAX:
            raise ValueError('Введенное число больше максимального',
                             Roman.ARABIC_MAX)
    ...
    def __check_roman(value):
        ...
        for i in value:
            if i not in Roman.LETTERS:
                raise ValueError('Число содержит недопустимые символы')

    @property
    def arabic(self):
        """Вернуть арабское представление числа."""
        if isinstance(self._arabic, int):
            return self._arabic
        elif isinstance(self._arabic, str):
            return Roman.to_arabic(self._arabic)
